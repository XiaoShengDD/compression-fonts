<template>
  <div id="card">
    ...
    <canvas width="325" height="1000" id="myCanvas">
...
export default {
  ...
  methods: {
    ...
    get_qrcode() {   //生成工点二维码  及验证密码   --- 前端
      QRCode.toDataURL(this.text, {
        ...
        this.setCanvas({
          ...
          coName: '鹰硕集团（深圳）有限公司',
          head: 'https://www.runoob.com/try/demo_source/img_the_scream.jpg',
          link: '官网：www.ys100.com1111111111111111111111111111111111111111111',
    ...
    setCanvas(options) {
      ...
      head.onload = () => {
        this.drawRoundRect(ctx, 8, 20, 30, 66, 66, head)
      };
      ...
      ctx.fillText(options.name, 100, 48);
      // 当前名称宽度
      ...
      ctx.fillText(options.phone, 100, 74);
      // 计算职位x轴距离
      let x = 110 + nameWidth
      ctx.fillText(options.position, x, 50);
      // 渲染当前邮箱，并且返回超出行数 计算邮箱超出高度
      let emailRowHeight = (this.drawtext(ctx, options.email, 100, 74, 205) * 20)

      // 横线
      ctx.moveTo(0, 125 + emailRowHeight);
      ctx.lineTo(325, 125 + emailRowHeight);
      ctx.lineWidth = 1
      ...
      let coNameRowHeight = (this.drawtext(ctx, options.coName, 20, (381 + emailRowHeight), 285, 25) * 25) + emailRowHeight
      ...
      ctx.fillText(options.tel, 20, 440 + coNameRowHeight);
      let linkRowHeight = (this.drawtext(ctx, options.link, 20, (440 + coNameRowHeight), 285) * 20) + coNameRowHeight
      let addressRowHeight = (this.drawtext(ctx, options.address, 20, (460 + linkRowHeight), 285) * 20) + linkRowHeight

      // 底部文字
      ctx.font = "15px sans-serif";
      ctx.fillStyle = "#909198"; // text color
      ctx.textAlign = "center";
      ctx.fillText("长按图片识别二维码或扫一扫，添加名片", 162.5, 520 + addressRowHeight);
      ...
      // 修改画布高度
      let canvasData = ctx.getImageData(0, 0, 325, 560 + addressRowHeight);  // 保存画布
      myCanvas.height = 560 + addressRowHeight;
      ...
      qrCode.onload = () => {
        ctx.drawImage(qrCode, 62.5, 155 + emailRowHeight);
